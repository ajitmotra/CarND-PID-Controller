...
int main()
{
  ...
  PID pid;
  double Kp0 = 0.13;  //0.12 //1.0;
  double Ki0 = 0.09;
  double Kd0 = 1.5;  
  double alpha_p = 0.0001; //0.00;
  double alpha_i = 0.00025;
  double alpha_d = 0.00001; //0.00;
  double  v = 0; 
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          pid.modifCoeffs(pid.Kp0, pid.Ki0, pid.Kd0, pid.alpha_p, pid.alpha_i,pid.alpha_d, speed);
          pid.UpdateError(cte);
          ...
          // DEBUG
#ifdef LOG_ENB
          std::cout << "CTE: " << cte << " dcte " << pid.cte_ - pid.cte_prev_ << " i_error " << pid.i_error << " p_error " << pid.p_error << std::endl;
#endif
          json msgJson;
